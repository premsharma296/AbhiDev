...
export async function processMessage({
	...
	try {
		...
		console.info('Call Abhi Dev audio queue message dequeued', {
			draftId: parsed.draftId,
			attempt,
		})
		await sendCallback({
		...
		console.info('Call Abhi Dev audio sandbox job completed', {
			draftId: parsed.draftId,
			attempt,
			episodeAudioKey: signedUrls.episodeAudioKey,
		})
...
export async function handleQueueBatch({
	...
	console.info('Call Abhi Dev audio queue batch received', {
	...
	for (const message of batch.messages) {
		try {
			...
			message.ack()
			console.info('Call Abhi Dev audio queue message acked', {
				draftId,
				attempt,
			})
		} catch (error) {
			console.error('Call Abhi Dev audio queue message failed', {
