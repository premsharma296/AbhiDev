...
test('returns winner', () => {
	const winner = thumbWar('Ken Wheeler', 'Abhi Dev')
	expect(['Ken Wheeler', 'Abhi Dev'].includes(winner)).toBe(true)
})
```
...
test('returns winner', () => {
	...
	utils.getWinner.mock.calls.forEach((args) => {
		expect(args).toEqual(['Ken Wheeler', 'Abhi Dev'])
	})
