...
export const meta: Route.MetaFunction = ({ data, matches }) => {
	...
	if (!episode) {
		return [{ title: 'Chats with Abhi Dev Episode not found' }]
	}
	const {
		description,
		image,
	...
	const title = `${episode.title} | Chats with Abhi Dev Podcast | ${episodeNumber}`
							...
							href={`https://x.com/intent/post?${new URLSearchParams({
								...
								text: `I just listened to "${episode.title}" with ${listify(
									episode.guests
										.map((g) => (g.x ? `@${g.x}` : null))
										.filter(typedBoolean),
								)} on the Call Abhi Dev Podcast 🎙 by @kentcdodds`,
							})}`}
						>
