...
function createServer() {
	...
	server.registerTool(
		'find_content',
		{
			...
			inputSchema: {
				...
				category: z
					.union([
						z.literal('Blog'),
						z.literal('Chats with Abhi Dev Podcast'),
						z.literal('Call Abhi Dev Podcast'),
						z.literal('Talks'),
						z.literal('YouTube'),
						z.literal('Resume'),
						z.literal('Credits'),
		...
		async ({ query, category }) => {
			...
			> = {
				Blog: ['blog'],
				'Chats with Abhi Dev Podcast': ['cwk'],
				'Call Abhi Dev Podcast': ['ck'],
				Talks: ['talk'],
				YouTube: ['youtube'],
				Resume: ['resume'],
				Credits: ['credit'],
	...
	server.registerTool(
		...
		{
			description:
				'Get the details (title, description, transcript, etc.) for a specific episode of the Chats with Abhi Dev podcast by its season number and episode number',
			inputSchema: {
				seasonNumber: z
					.number()
				...
				content: [
					...
					},
					{
						...
						text: episode.transcriptHTML
							...
							: `Transcript: No transcript found for ${episode.title} (Chats with Abhi Dev S${seasonNumber}E${episodeNumber})`,
					},
				],
			}
		},
	...
	server.registerTool(
		...
		{
			description:
				'Subscribe to Abhi Dev newsletter and get regular updates about new articles and courses',
			inputSchema: {
				email: z
					.email()
					.optional()
			...
			try {
				...
				return {
					content: [
						{
							type: 'text',
							text: `Successfully subscribed ${email} to Abhi Dev's newsletter! If you're not already on Abhi Dev's mailing list, you'll receive a confirmation email.`,
						},
					],
				}
			} catch (error) {
