		...
		.map((favorite): FavoriteDisplayItem | null => {
			switch (favorite.contentType) {
				...
				case 'call-kent-episode': {
					...
					const title = episode?.title ?? `Call Abhi Dev Episode`
					return {
				...
				case 'chats-with-kent-episode': {
					...
					const title = episode?.title ?? `Chats with Abhi Dev Episode`
					return {
	...
	const callKentCallerEpisodesDisplay = callKentCallerEpisodes.map((entry) => {
		...
		if (!episode) {
			return {
				...
				episodeNumber: null,
				slug: '',
				episodeTitle: 'Call Abhi Dev episode (unavailable)',
				episodePath: '/calls',
				imageUrl: null,
...
function YouScreen({ loaderData: data, actionData }: Route.ComponentProps) {
	...
	return (
		<main>
			...
			<Grid>
				<div className="col-span-full mb-12">
					<H2>Your Call Abhi Dev episodes</H2>
					<H2 variant="secondary" as="p">
