...
function getStaticDocs(): SearchDoc[] {
	return [
		...
		},
		{
			...
			content: 'Call Abhi Dev Podcast episodes.',
		},
	]
}
...
export const cloudflareHandlers: Array<HttpHandler> = [
	...
	http.post<any, DefaultBodyType>(
		...
		async ({ request }) => {
			...
			// Text generation / chat models (used for Call Abhi Dev metadata generation).
			...
			if (hasMessages || hasPrompt) {
				...
				return jsonOk({
					response: JSON.stringify({
						title: `Mock Call Abhi Dev episode title (${model})`,
						description:
